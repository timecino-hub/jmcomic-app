# iPadOS 11 / 13 英寸适配与安装

本分支把原先仅声明 iPhone 的目标改成 **iPhone + iPad 通用 App**，最低系统仍为 iOS / iPadOS 17。界面按可用宽度响应，而不是写死某一代设备的像素，因此覆盖 11 英寸和 13 英寸 iPad 的横屏、竖屏、分屏和台前调度窗口。

## 已适配内容

- 11 英寸竖屏默认收起侧边栏，漫画区稳定显示四列；左上角系统按钮可随时展开侧栏。
- 13 英寸与 11 英寸保持相同信息密度：竖屏四列、横屏五列；额外空间用于放大封面、标题、作者信息和详情页封面，避免内容过密。
- 横屏宽窗口使用 iPad 侧边栏；不足 900 pt 的窗口优先显示完整内容，紧凑窗口自动切回底部标签栏。
- 支持 iPad 四个方向和多窗口。
- 网格在 4:3 宽屏上自动增加列数，详情页限制阅读行宽。
- 连续阅读页按窗口短边自适应：11 英寸上限 900 pt，13 英寸上限 1024 pt；单页模式仍完整适配整个视口。
- 阅读器顶栏和跳页条在宽屏居中限宽。
- 支持触控板 / 鼠标的间接输入事件。
...
在 iPad 13 英寸模拟器上另外检查：

- 竖屏全屏：默认隐藏侧边栏 + 四列内容网格，封面应明显大于 11 英寸。
- 横屏全屏：侧边栏 + 五列内容网格，额外宽度用于增大卡片而不是增加列数。
- 在线与本地连续阅读：页面最大宽度约 1024 pt，居中且两侧留白对称。

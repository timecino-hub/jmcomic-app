...
final class LibraryStore: ObservableObject {
    ...
    /// albums 字典加上限：最多 200 条，LRU 淘汰最旧的
    private var albums: [String: Album] = [:]
    private var albumOrder: [String] = []
    private let albumsLimit = 200

    /// saveState 防抖：翻页时不立即写盘，2 秒内多次写入合并为一次
        ...
        if stateTimer == nil {
            stateTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: false) { [weak self] _ in
                Task { @MainActor [weak self] in
                    self?.stateTimer = nil
                    guard self?.stateDirty == true else { return }
    ...
    func cache(_ album: Album) {
        // 如果已存在，先移除旧位置
        if let idx = albumOrder.firstIndex(of: album.id) {
            albumOrder.remove(at: idx)
        }
        albums[album.id] = album
        albumOrder.append(album.id)

        // LRU 淘汰：超出上限时移除最旧的
        while albumOrder.count > albumsLimit {
            let oldest = albumOrder.removeFirst()
            albums.removeValue(forKey: oldest)
        }
        saveAlbums()
    }
